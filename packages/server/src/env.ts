...
/** Exact URL or regular expression accepted by the HTTP CORS middleware. */
export type CorsOrigin = string | RegExp;

/** Runtime configuration resolved from server environment variables. */
...
/** Loads and validates server environment variables into typed runtime config. */
export function loadEnv(source: NodeJS.ProcessEnv = process.env): ServerEnv {
  const env = loadConfig(envSchema, source);

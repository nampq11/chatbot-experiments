...
import { createMigrateProgram, MIGRATION_TABLES, runMigrateDown, runMigrateUp } from "./migrate.ts";

const mocks = vi.hoisted(() => {
  const runMigrations = vi.fn(async () => {});
  const resetDatabaseSchema = vi.fn(async () => {});
  const migrationTables = ["agent_runs", "messages", "sessions", "users", "__drizzle_migrations"] as const;

  return { runMigrations, resetDatabaseSchema, migrationTables };
});

vi.mock("@dentaltrip-ai/database", () => ({
...
describe("createMigrateProgram", () => {
  it("exposes up and down commands", async () => {
    ...
    await program.parseAsync(["node", "dentaltrip-ai", "up"]);
    await program.parseAsync(["node", "dentaltrip-ai", "down"]);
